...
- [x] `supp config` / `supp project` — compact repo fingerprint: languages, frameworks, package managers, test runners, commands, entrypoints, instruction files
- [x] `supp commands` / `supp cmds` — discovered build/test/lint/eval command surface
- [x] `supp tests` / `supp tests-for` / `supp test-map` — likely test files and focused validation commands for a file, symbol, or changed files
- [x] `supp docs` — search docs, docstrings, and inline comments for contextual hints
- [x] `supp review` — reviewer-focused diff packet with related tests and docs/comments context
